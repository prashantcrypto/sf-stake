    ...
    const txs: (Transaction | null)[] = await Promise.all(
      stakedSelected.map(async (token) => {
        try {
          ...
          notify({
            message: `${e}`,
            description: `Failed to unstake goat ${token?.stakeEntry?.pubkey.toString()}`,
            type: 'error',
          })
          return null
        }
  ...
  async function handleStake() {
    ...
    for (let step = 0; step < unstakedSelected.length; step++) {
      try {
        ...
        notify({
          message: `Failed to stake goat ${unstakedSelected[
            step
    ...
    const txs: (Transaction | null)[] = await Promise.all(
      unstakedSelected.map(async (token) => {
        try {
          ...
          notify({
            message: `Failed to unstake goat ${token?.stakeEntry?.pubkey.toString()}`,
            description: `${e}`,
            type: 'error',
          })
          return null
  ...
  const totalStakedTokens = async () => {
    ...
    for (const entry of stakePoolEntries.data) {
      try {
        ...
        console.log('Error calculating total staked goats', e)
      }
    }
        ...
        <div className="my-2 mx-5 grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          >
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <MouseoverTooltip title="Click on goats to select them">
                <button
                  onClick={() => {
                    if (unstakedSelected.length === 0) {
                      notify({
                        message: `No goats selected`,
                        type: 'error',
                      })
                    } else {
                      handleStake()
                ...
                >
                  ...
                  <span className="my-auto">
                    Stake Goats ({unstakedSelected.length})
                  </span>
                </button>
              </MouseoverTooltip>
            </div>
            ...
            <div className="mt-2 flex flex-row-reverse">
              ...
              >
                ...
                >
                  ...
                  <span className="my-auto">
                    Unstake Goats ({stakedSelected.length})
                  </span>
                </button>
              </MouseoverTooltip>
              {rewardDistributorData.data &&
